...
//    void initialize() {
//        //INITIALIZE MOCKS
//        plcService = spy(new PlcServiceImpl(plcRepository, plcConnector, curveValidationService, referenceCurveCalculationService, logService, logCreator));
//
...
//            when(plcConnector.getPlcData(testPlc.getIpAddress())).thenReturn(client);
//
...
//            verify(plcConnector, times(0)).getClient(any());
//        }
//
...
//                SetBehaviour.correctlyConnectsOverOpcUa(plcConnector);
//            }
//
...
//                plcService.createPlc(newPlc);
//
//                verify(plcConnector, Mockito.times(1)).connectPlc(newPlc);
//            }
//
...
//
//                when(plcConnector.connectPlc(plcToBeConnected)).thenThrow(new OpcuaConnectionException());
//
...
//            when(plcConnector.connectPlc(any())).thenThrow(new OpcuaConnectionException());
//
//            plcService.connectAllPlcsInDatabase();
//
//            verify(plcConnector,times(2)).connectPlc(any(Plc.class));
//        }
//    }
//
...
//                plcService.delete(idOfExistingPlc);
//
//                verify(plcConnector,Mockito.times(1)).disconnectPlc(plcInDb);
//            }
//        }
//
...
//
//                verify(plcConnector,Mockito.times(1)).disconnectPlc(plcCaptor.capture());
//                assertThat(plcCaptor.getValue().getIpAddress()).isEqualTo("192.168.0.1");
//                verify(plcConnector,Mockito.times(1)).connectPlc(plcCaptor.capture());
